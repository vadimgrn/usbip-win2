...
PAGED auto initialize(_Inout_ WDFDEVICE_INIT *init)
{
        ...
        /*
         * Security Note:
         * SDDL_DEVOBJ_SYS_ALL_ADM_RWX_WORLD_RW_RES_R grants World (Everyone / unprivileged users)
         * Read and Write access (D:P(A;;GA;;;SY)(A;;GRGWGX;;;BA)(A;;GRGW;;;WD)(A;;GR;;;RC)).
         * This is an intentional usability design choice for desktop client scenarios: any local user
         * can attach/detach remote USB devices via usbip/wusbip without requiring UAC elevation,
         * matching the physical USB plug/unplug user experience.
         *
         * Security trade-offs:
         * 1. Kernel-mode outbound connection primitive: PLUGIN_HARDWARE allows unprivileged callers
         *    to instruct the driver (running as SYSTEM in kernel mode) to open outbound TCP connections
         *    to arbitrary hosts/ports (potential SSRF / network reconnaissance).
         * 2. System persistence modification: SET_PERSISTENT allows unprivileged callers to write
         *    arbitrary persistent device lists into HKLM (see set_persistent in vhci_ioctl.cpp).
         * 3. Cross-user device manipulation: on multi-user systems, callers can disconnect or abort
         *    attach attempts for devices imported by other users (no per-caller ownership check).
         *
         * How 1 (PLUGIN_HARDWARE / outbound connection primitive) can be restricted:
         * - Device ACL: Change SDDL to SDDL_DEVOBJ_SYS_ALL_ADM_RWX_WORLD_R (World Read-only, Admins RWX),
         *   requiring administrative privilege to open the device for write/IOCTL dispatch.
         * - Caller privilege check: Verify caller holds administrative privileges or specific rights
         *   (e.g., SeSinglePrivilegeCheck for SE_LOAD_DRIVER_PRIVILEGE or token membership in Administrators)
         *   in plugin_hardware() before initiating connection.
         * - Caller impersonation: Use WdfRequestImpersonate() before socket connect so that outbound
         *   network traffic is attributed to the user's security token and evaluated against user-level
         *   firewall / Windows Filtering Platform (WFP) rules rather than kernel SYSTEM context.
         * - Destination filtering: Validate target host/port in plugin_hardware() (e.g., enforce
         *   standard USB/IP port 3240, block loopback 127.0.0.1 / ::1, link-local, or private subnets).
         */
