...
PAGED NTSTATUS vhci_query_remove(_In_ WDFDEVICE vhci)
{
        ...
        vhci::detach_all_devices(NULL_SESSION_ID, vhci, true);
...
_IRQL_requires_same_
_IRQL_requires_(PASSIVE_LEVEL)
PAGED void usbip::vhci::detach_all_devices(_In_ ULONG session_id, _In_ WDFDEVICE vhci, _In_ bool async)
{
        PAGED_CODE();

        TraceDbg("session id %lx", session_id);
        auto &ctx = *get_vhci_ctx(vhci);
        ...
        for (int port = 1; port <= ctx.devices_cnt; ++port) {
                ...
                if (auto dev = get_device(vhci, port); auto hdev = dev.get<UDECXUSBDEVICE>()) {
                        ...
                        if (auto dc = get_device_ctx(hdev);
                            auto err = check_session_access(session_id, dc->owner_session_id)) {

                                Trace(TRACE_LEVEL_INFORMATION, "port %d, owner session id %lx, %!STATUS!",
                                        port, dc->owner_session_id, err);
