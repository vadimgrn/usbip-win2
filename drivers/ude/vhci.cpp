...
PAGED auto create_bus(_Inout_ WDFDEVICE_INIT *init)
{
...

        {
                ...
                WDF_CHILD_LIST_CONFIG_INIT(&cfg, sizeof(session_hc_description), child_list_create_device);
                WdfFdoInitSetDefaultChildListConfig(init, &cfg, WDF_NO_OBJECT_ATTRIBUTES);
        }

        WDFDEVICE bus{};
        if (auto err = WdfDeviceCreate(&init, WDF_NO_OBJECT_ATTRIBUTES, &bus)) {
                Trace(TRACE_LEVEL_ERROR, "WdfDeviceCreate %!STATUS!", err);
                return err;
        }

        {
                ...
                WdfDeviceSetBusInformationForChildren(bus, &info);
        }

        {
                WDF_IO_QUEUE_CONFIG cfg;
                WDF_IO_QUEUE_CONFIG_INIT_DEFAULT_QUEUE(&cfg, WdfIoQueueDispatchSequential);
                cfg.EvtIoDeviceControl = bus_control;

                if (auto err = WdfIoQueueCreate(bus, &cfg, WDF_NO_OBJECT_ATTRIBUTES, WDF_NO_HANDLE)) {
                        Trace(TRACE_LEVEL_ERROR, "WdfIoQueueCreate %!STATUS!", err);
                        return err;
                }
        }
        ...
        Trace(TRACE_LEVEL_INFORMATION, "%04x", ptr04x(bus));
        return STATUS_SUCCESS;
}

/*
 * Create UDE host controller device aka VHCI.
 */
_IRQL_requires_same_
_IRQL_requires_(PASSIVE_LEVEL)
PAGED auto create_ude(_Inout_ WDFDEVICE_INIT *init)
{
        ...
        if (ULONG session_id; NT_SUCCESS(get_session_id(session_id, vhci))) {
                Trace(TRACE_LEVEL_INFORMATION, "session id %lx", session_id);
        }
...
PAGED auto is_ude(_Inout_ bool &result, _In_ WDFDEVICE_INIT *init)
{
        ...
        WDFMEMORY mem;
        if (auto err = WdfFdoInitAllocAndQueryProperty(
                init, DevicePropertyHardwareID, PagedPool, WDF_NO_OBJECT_ATTRIBUTES, &mem)) {
                Trace(TRACE_LEVEL_ERROR, "WdfFdoInitAllocAndQueryProperty(HardwareID) %!STATUS!", err);
                return err;
        }

        auto buf = (const wchar_t*)WdfMemoryGetBuffer(mem, nullptr);
