...
setup_with_dsc_conf(pctx_vusb_t vusb, PUSB_CONFIGURATION_DESCRIPTOR dsc_conf)
{
	...
	if (vusb->dsc_conf == NULL) {
		TraceError(PLUGIN, "failed to allocate configuration descriptor");
		return FALSE;
	}
	...
	if (dsc_conf->bNumInterfaces > 0) {
		...
		if (vusb->intf_altsettings == NULL) {
			TraceError(PLUGIN, "failed to allocate alternative settings for interfaces");
			return FALSE;
		}
...
setup_vusb(UDECXUSBDEVICE ude_usbdev, pvhci_pluginfo_t pluginfo)
{
	...
	if (NT_ERROR(status)) {
		TraceError(PLUGIN, "failed to create wait lock: %!STATUS!", status);
		return FALSE;
	}
	...
	if (NT_ERROR(status)) {
		TraceError(PLUGIN, "failed to create urbr memory: %!STATUS!", status);
		return FALSE;
	}
	...
	if (!setup_with_dsc_conf(vusb, &pluginfo->dscr_conf)) {
		TraceError(PLUGIN, "failed to setup usb with configuration descritor");
		return FALSE;
	}
...
setup_descriptors(PUDECXUSBDEVICE_INIT pdinit, pvhci_pluginfo_t pluginfo)
{
	...
	if (NT_ERROR(status)) {
		TraceWarning(PLUGIN, "failed to add a device descriptor to device init");
	}

	status = UdecxUsbDeviceInitAddDescriptor(pdinit, (UCHAR*)&pluginfo->dscr_conf, pluginfo->dscr_conf.wTotalLength);
		if (NT_ERROR(status)) {
		TraceWarning(PLUGIN, "failed to add a configuration descriptor to device init");
	}
}
...
create_endpoints(UDECXUSBDEVICE ude_usbdev, vhci_pluginfo_t *pluginfo)
{
	...
	TraceInfo(VUSB, "Enter: epinit=0x%p", epinit);
	...
	for (USB_COMMON_DESCRIPTOR *cur = NULL; 
		...
		TraceInfo(VUSB, "epinit=%#p, bEndpointAddress=%#04x",	epinit, d->bEndpointAddress);
...
static UDECX_USB_DEVICE_SPEED get_device_speed(pvhci_pluginfo_t pluginfo)
{
	...
	default:
		TraceError(PLUGIN, "unknown bcdUSB:%x", (ULONG)bcdUSB);
		return UdecxUsbLowSpeed;
	}
}
...
vusb_plugin(pctx_vhci_t vhci, pvhci_pluginfo_t pluginfo)
{
	...
	if (NT_ERROR(status)) {
		TraceError(PLUGIN, "failed to create usb device: %!STATUS!", status);
	...
	if (NT_ERROR(status)) {
		TraceError(PLUGIN, "failed to plugin a new device %!STATUS!", status);
		WdfObjectDelete(ude_usbdev);
		return NULL;
	}
