...
fetch_urbr_control_transfer(PURB urb, struct usbip_header *hdr)
{
	...
	if (IsTransferDirectionOut(urb_ctltrans->TransferFlags)) {
		return STATUS_SUCCESS;
	}

	NTSTATUS status = copy_to_transfer_buffer(urb_ctltrans->TransferBuffer, urb_ctltrans->TransferBufferMDL,
		                                  urb_ctltrans->TransferBufferLength, hdr + 1, 
...
fetch_urbr_control_transfer_ex(PURB urb, struct usbip_header *hdr)
{
	...
	if (IsTransferDirectionOut(urb_ctltrans_ex->TransferFlags)) {
		return STATUS_SUCCESS;
	}

	NTSTATUS status = copy_to_transfer_buffer(urb_ctltrans_ex->TransferBuffer, urb_ctltrans_ex->TransferBufferMDL,
		                                  urb_ctltrans_ex->TransferBufferLength, hdr + 1, 
						  hdr->u.ret_submit.actual_length);
