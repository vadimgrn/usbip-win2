...
store_urbr_partial(WDFREQUEST req_read, purb_req_t urbr)
{
	...
	TraceInfo(READ, "Enter: urbr: %!URBR!", urbr);
	...
	case URB_FUNCTION_ISOCH_TRANSFER:
		...
		break;
	default:
		TraceWarning(READ, "unexpected partial urbr: %!URBFUNC!", urbfunc);
...
store_urbr_urb(WDFREQUEST req_read, purb_req_t urbr)
{
	...
	default:
		...
		TraceError(READ, "unhandled urb function: %!URBFUNC!", urb_func);
		status = STATUS_INVALID_PARAMETER;
		break;
	}
...
{
	TraceInfo(READ, "urbr: %s", dbg_urbr(urbr));
	...
	case URBR_TYPE_RESET_PIPE:
		...
		break;
	default:
		TraceError(READ, "unknown type: %d", urbr->type);
