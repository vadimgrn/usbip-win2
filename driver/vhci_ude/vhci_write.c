...
write_vusb(pctx_vusb_t vusb, WDFREQUEST req_write)
{
	...
	TraceInfo(WRITE, "Enter");

	hdr = get_hdr_from_req_write(req_write);
	if (hdr == NULL) {
		TraceError(WRITE, "small write irp\n");
	...
	if (urbr == NULL) {
		...
		TraceWarning(WRITE, "no urbr: seqnum: %d", hdr->base.seqnum);
...
io_write(_In_ WDFQUEUE queue, _In_ WDFREQUEST req, _In_ size_t len)
{
	...
	TraceInfo(WRITE, "Enter: len: %u", (ULONG)len);

	vusb = get_vusb_by_req(req);
	if (vusb == NULL) {
		TraceInfo(WRITE, "vusb disconnected: port: %u", TO_SAFE_VUSB_FROM_REQ(req)->port);
