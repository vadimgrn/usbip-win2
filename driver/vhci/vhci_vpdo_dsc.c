...
req_fetch_dsc(pvpdo_dev_t vpdo, PIRP irp)
{
	...
	if (urbr == NULL)
		...
		if (NT_SUCCESS(status))
			...
			TraceInfo(DBG_GENERAL, "failed to submit unlink urb: %s\n", dbg_urbr(urbr));
			free_urbr(urbr);
...
PAGEABLE NTSTATUS vpdo_get_dsc_from_nodeconn(vpdo_dev_t *vpdo, IRP *irp, USB_DESCRIPTOR_REQUEST *dsc_req, ULONG *psize)
{
	...
	case USB_STRING_DESCRIPTOR_TYPE:
		...
		break;
	default:
		TraceError(DBG_GENERAL, "unhandled descriptor type: %s\n", dbg_usb_descriptor_type(setup->wValue.HiByte));
		break;
	}
...
static BOOLEAN need_caching_dsc(vpdo_dev_t *vpdo, struct _URB_CONTROL_DESCRIPTOR_REQUEST *urb_cdr, USB_COMMON_DESCRIPTOR *dsc)
{
	...
	case USB_CONFIGURATION_DESCRIPTOR_TYPE:
		...
		if (urb_cdr->TransferBufferLength < dsc_conf->wTotalLength) {
			TraceInfo(DBG_WRITE, "ignore non-full configuration descriptor\n");
			return FALSE;
		}
		break;
	case USB_STRING_DESCRIPTOR_TYPE:
	...
	USB_COMMON_DESCRIPTOR *dsc)
{
	...
	if (vpdo->serial) {
		TraceWarning(DBG_VPDO, "prior serial '%S'\n", vpdo->serial);
	...
	if (vpdo->serial) {
		...
		if (st == STATUS_SUCCESS) {
			TraceInfo(DBG_VPDO, "serial '%S', LangId %#04x\n", vpdo->serial, urb_cdr->LanguageId);
		} else {
			NT_ASSERT(!"RtlStringCchCopyNW failed");
		}
	} else {
		TraceError(DBG_VPDO, "can't allocate memory: Index %d, bLength %d, LangId %#04x\n",
	...
	USB_COMMON_DESCRIPTOR* dsc)
{
	...
	if (!dsc_new) {
		TraceError(DBG_WRITE, "out of memory\n");
		return;
	}
