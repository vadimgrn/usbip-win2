...
static PAGEABLE void *get_urb_buffer(void *buf, MDL *bufMDL)
{
	...
	if (!bufMDL) {
		TraceError(FLAG_GENERAL, "TransferBuffer and TransferBufferMDL are NULL");
		return nullptr;
	}
...
static PAGEABLE NTSTATUS urb_function_generic(vpdo_dev_t *vpdo, URB *urb, const struct usbip_header *hdr)
{
	...
	if (buf && !bulk) { // bulk can have sensitive data
		TraceUrb("%s(%#04x): %!BIN!", urb_function_str(func), func, 
...
static PAGEABLE NTSTATUS urb_control_descriptor_request(vpdo_dev_t *vpdo, URB *urb, const struct usbip_header *hdr)
{
	...
	if (r->TransferBufferLength < sizeof(*dsc)) {
		TraceError(FLAG_GENERAL, "usb descriptor's header expected: TransferBufferLength(%lu)", r->TransferBufferLength);
		r->TransferBufferLength = 0;
	...
	const struct usbip_iso_packet_descriptor *src, const char *src_buf, ULONG src_len)
{
	...
	for (ULONG i = 0; i < r->NumberOfPackets; ++i, ++src, ++dst) {
		...
		if (src->actual_length > src->length) {
			TraceError(FLAG_GENERAL, "src->actual_length(%u) > src->length(%u)", src->actual_length, src->length);
			return STATUS_INVALID_PARAMETER;
		}

		if (src->offset != dst->Offset) { // buffer is compacted, but offsets are intact
			TraceError(FLAG_GENERAL, "src->offset(%u) != dst->Offset(%lu)", src->offset, dst->Offset);
			return STATUS_INVALID_PARAMETER;
		}

		if (src_offset > dst->Offset) {// source buffer has no gaps
			TraceError(FLAG_GENERAL, "src_offset(%lu) > dst->Offset(%lu)", src_offset, dst->Offset);
			return STATUS_INVALID_PARAMETER;
		}

		if (src_offset + src->actual_length > src_len) {
			TraceError(FLAG_GENERAL, "src_offset(%lu) + src->actual_length(%u) > src_len(%lu)", 
		...
		if (dst->Offset + src->actual_length > r->TransferBufferLength) {
			TraceError(FLAG_GENERAL, "dst->Offset(%lu) + src->actual_length(%u) > r->TransferBufferLength(%lu)", 
	...
	if (!ok) {
		TraceError(FLAG_GENERAL, "src_offset(%lu) != src_len(%lu)", src_offset, src_len);
	}
...
static PAGEABLE NTSTATUS urb_isoch_transfer(vpdo_dev_t *vpdo, URB *urb, const struct usbip_header *hdr)
{
	...
	if (!(cnt >= 0 && (ULONG)cnt == r->NumberOfPackets)) {
		TraceError(FLAG_GENERAL, "number_of_packets(%d) != NumberOfPackets(%lu)", cnt, r->NumberOfPackets);
		return STATUS_INVALID_PARAMETER;
	}

	if (!(res->actual_length >= 0 && (ULONG)res->actual_length <= r->TransferBufferLength)) {
		TraceError(FLAG_GENERAL, "actual_length(%d) > TransferBufferLength(%lu)", res->actual_length, r->TransferBufferLength);
		return STATUS_INVALID_PARAMETER;
	}
...
static PAGEABLE NTSTATUS usb_submit_urb(vpdo_dev_t *vpdo, URB *urb, const struct usbip_header *hdr)
{
	...
	if (urb) {
		...
		TraceInfo(FLAG_GENERAL, "URB is null");
		return STATUS_INVALID_PARAMETER;
	}
	...
	if (err && !urb->UrbHeader.Status) {
		...
		TraceVerbose(FLAG_GENERAL, "Set USBD_STATUS=%s because return is %!STATUS!", dbg_usbd_status(urb->UrbHeader.Status), err);
	} else if (urb->UrbHeader.Status && !err) {
		err = STATUS_UNSUCCESSFUL;
		TraceVerbose(FLAG_GENERAL, "Return %!STATUS! because USBD_STATUS=%s(%#08lX)", 
...
static PAGEABLE NTSTATUS get_descriptor_from_node_connection(IRP *irp, const struct usbip_header *hdr)
{
	...
	if (!(actual_length >= 0 && (ULONG)actual_length <= data_sz)) {
		TraceError(FLAG_GENERAL, "OutputBufferLength %lu, actual_length(%d)", data_sz, actual_length);
		return STATUS_INVALID_PARAMETER;
	}
	...
	TraceVerbose(FLAG_GENERAL, "ConnectionIndex %lu, Data[%!BIN!]", r->ConnectionIndex, WppBinary(r->Data, (USHORT)actual_length));
...
static PAGEABLE NTSTATUS ret_submit(struct urb_req *urbr, const struct usbip_header *hdr)
{
	...
	case IOCTL_USB_GET_DESCRIPTOR_FROM_NODE_CONNECTION:
		...
		break;
	default:
		TraceError(FLAG_GENERAL, "Unexpected IoControlCode %s(%#08lX)", dbg_ioctl_code(ioctl_code), ioctl_code);
	}
...
static PAGEABLE const struct usbip_header *consume_write_irp_buffer(IRP *irp)
{
	...
	if (buf_sz < sizeof(*hdr)) {
		TraceError(FLAG_GENERAL, "usbip header expected: write buffer %lu", buf_sz);
		return nullptr;
	}
...

	{
		...
		TraceInfo(FLAG_USBIP, "IN %Iu%s", pdu_sz, dbg_usbip_hdr(buf, sizeof(buf), hdr));
	}
	...
	TraceError(FLAG_GENERAL, "Write buffer %lu != pdu size %Iu", buf_sz, pdu_sz);
	return nullptr;
}
...
static PAGEABLE NTSTATUS do_write_irp(vpdo_dev_t *vpdo, IRP *write_irp)
{
	...
	if (!urbr) {
		TraceVerbose(FLAG_GENERAL, "urb_req not found (cancelled?), seqnum %u", hdr->base.seqnum);
		return STATUS_SUCCESS;
	}
	...
	case USBIP_RET_UNLINK:
		...
		break;
	default:
		TraceError(FLAG_GENERAL, "USBIP_RET_* expected, got %!usbip_request_type!", hdr->base.command);
	}
...
extern "C" PAGEABLE NTSTATUS vhci_write(__in DEVICE_OBJECT *devobj, __in IRP *irp)
{
	...
	TraceVerbose(FLAG_GENERAL, "Enter irql %!irql!, write buffer %lu", KeGetCurrentIrql(), get_irp_buffer_size(irp));

	vhci_dev_t *vhci = devobj_to_vhci_or_null(devobj);
	if (!vhci) {
		TraceWarning(FLAG_GENERAL, "write for non-vhci is not allowed");
	...
	if (vhci->common.DevicePnPState != Deleted) {
		...
		if (vpdo && vpdo->plugged) {
			...
			TraceVerbose(FLAG_GENERAL, "null or unplugged");
