...
start_vhci(pvhci_dev_t vhci)
{
	...
	if (!NT_SUCCESS(status)) {
		TraceError(TRACE_PNP, "failed to register vhci device interface: %s\n", dbg_ntstatus(status));
		return status;
	}
	status = IoRegisterDeviceInterface(vhci->common.pdo, (LPGUID)&GUID_DEVINTERFACE_USB_HOST_CONTROLLER, NULL, &vhci->DevIntfUSBHC);
	if (!NT_SUCCESS(status)) {
		TraceError(TRACE_PNP, "failed to register USB Host controller device interface: %s\n", dbg_ntstatus(status));
		return status;
	}
	...
	status = reg_wmi(vhci);
	if (!NT_SUCCESS(status)) {
		TraceError(TRACE_VHCI, "start_vhci: reg_wmi failed: %s\n", dbg_ntstatus(status));
	}
...
start_vhub(pvhub_dev_t vhub)
{
	...
	if (NT_ERROR(status)) {
		TraceError(TRACE_PNP, "failed to register USB root hub device interface: %s\n", dbg_ntstatus(status));
		return STATUS_UNSUCCESSFUL;
	}
	status = IoSetDeviceInterfaceState(&vhub->DevIntfRootHub, TRUE);
	if (NT_ERROR(status)) {
		TraceError(TRACE_PNP, "failed to activate USB root hub device interface: %s\n", dbg_ntstatus(status));
		return STATUS_UNSUCCESSFUL;
	}
	...
	if (!NT_SUCCESS(status)) {
		TraceError(TRACE_PNP, "failed to enable vhci device interface: %s\n", dbg_ntstatus(status));
		return status;
	}
	status = IoSetDeviceInterfaceState(&vhci->DevIntfUSBHC, TRUE);
	if (!NT_SUCCESS(status)) {
		IoSetDeviceInterfaceState(&vhci->DevIntfVhci, FALSE);
		TraceError(TRACE_PNP, "failed to enable USB host controller device interface: %s\n", dbg_ntstatus(status));
		return status;
	}
...
start_vpdo(pvpdo_dev_t vpdo)
{
	...
	if (NT_SUCCESS(status)) {
		...
		if (NT_ERROR(status)) {
			TraceWarning(TRACE_VPDO, "failed to activate USB device interface: %s\n", dbg_ntstatus(status));
		}
	}
	else {
		TraceWarning(TRACE_VPDO, "failed to register USB device interface: %s\n", dbg_ntstatus(status));
	}
