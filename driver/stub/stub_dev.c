...
create_devobj_idx(PDRIVER_OBJECT drvobj, ULONG devtype, int idx)
{
	...
	if (NT_ERROR(status)) {
		if (status != STATUS_OBJECT_NAME_COLLISION)
			TraceError(DBG_GENERAL, "create_devobj_idx: IoCreateDevice failed: %S: err: %s\n", devname, dbg_ntstatus(status));
		return NULL;
	}
	...
	if (NT_ERROR(status)) {
		TraceError(DBG_GENERAL, "create_devobj_idx: IoCreateSymbolicLink failed: %S: err: %s\n", devname, dbg_ntstatus(status));
...
is_addable_pdo(DEVICE_OBJECT *pdo)
{
	...
	if (id_hw == NULL) {
		TraceWarning(DBG_DISPATCH, "unable to get HW id from registry\n");
		return FALSE;
	}
	...
	if (strncmp(id_hw, "USB\\", 4) != 0 || strncmp(id_hw + 4, "VID_", 4) != 0 || strlen(id_hw + 8) < 4 ||
		...
		TraceInfo(DBG_DISPATCH, "skipping non-usb device or hub: %s\n", id_hw);
		ExFreePool(id_hw);
		return FALSE;
	}
	ExFreePool(id_hw);
	...
	if (id_compat == NULL) {
		TraceWarning(DBG_DISPATCH, "unable to get compatible id from registry\n");
		return FALSE;
	}

	// Don't attach to usb device hubs
	if (strncmp(id_compat, "USB\\Class_09", 12) == 0) {
		TraceInfo(DBG_DISPATCH, "skipping usb device hub: compatible id: %s\n", id_compat);
	...
	if (is_usbip_stub_attached(pdo)) {
		TraceInfo(DBG_DISPATCH, "skipping usbip stub\n");
		return FALSE;
	}
...
stub_add_device(PDRIVER_OBJECT drvobj, PDEVICE_OBJECT pdo)
{
	...
	TraceInfo(DBG_DEV, "add_device: %s\n", dbg_devices(pdo, TRUE));
	...
	if (devobj == NULL) {
		TraceError(DBG_DEV, "failed to create usbip stub device\n");
		return STATUS_SUCCESS;
	}
	...
	if (!reg_get_properties(devstub)) {
		TraceError(DBG_DEV, "failed to setup device from properties\n");
	...
	if (devstub->next_stack_dev == NULL) {
		TraceError(DBG_DISPATCH, "failed to attach: %s\n", dbg_devstub(devstub));
	...
	if (NT_ERROR(status)) {
		TraceError(DBG_DISPATCH, "add_device: failed to create USBD handle: %s: %s\n", dbg_devstub(devstub), dbg_ntstatus(status));
	...
	TraceInfo(DBG_DEV, "add_device: device added: %s\n", dbg_devstub(devstub));
