...
reg_get_property(PDEVICE_OBJECT pdo, int property)
{
	...
	if (status != STATUS_BUFFER_TOO_SMALL) {
		DBGE(DBG_GENERAL, "IoGetDeviceProperty failed to get size: status: %!STATUS!\n", status);
		return NULL;
	}
	...
	if (NT_ERROR(status)) {
		DBGE(DBG_GENERAL, "IoGetDeviceProperty failed: status: %x\n", status);
		ExFreePool(buf);
		return NULL;
	}
	...
	if (NT_ERROR(status)) {
		DBGE(DBG_GENERAL, "failed to convert unicode string: status: %x\n", status);
		return NULL;
	}
...
reg_get_properties(usbip_stub_dev_t *devstub)
{
	...
	if (NT_FAILED(status)) {
		DBGE(DBG_GENERAL, "IoOpenDeviceRegistryKey failed: status: %!STATUS!\n", status);
		return FALSE;
	}
	...
	if (info == NULL) {
		ZwClose(hkey);
		DBGE(DBG_GENERAL, "ExAllocatePool failed allocating %d bytes\n", pool_length);
		return FALSE;
	}
	...
	if (!dev->device_interface_in_use) {
		status = RtlGUIDFromString(&device_interface_guid_value, &dev->device_interface_guid);
		if (NT_SUCCESS(status)) {
			DBGI(GDB_GENERAL, "using default device interface guid\n");
			dev->device_interface_in_use = TRUE;
		}
		else {
			DBGE(GDB_GENERAL, "failed using default device interface guid\n");
		}
	}
