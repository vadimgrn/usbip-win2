 ...
 */
USBIP_API bool set_persistent(_In_ HANDLE dev, _In_ const std::vector<persistent_device> &devices);

/**
 * @param dev handle of the driver device
 * @return devices stashed by set_persistent() if the result contains a value, otherwise call GetLastError()
 */
USBIP_API std::optional<std::vector<persistent_device>> get_persistent(_In_ HANDLE dev);
